...
- Existing alias, identity, stream-offer polling/admission, lifecycle
  ...
  helper surfaces.
- The released scenario set runs contiguously from `001` through `066`, with
  scenarios `064` through `066` covering v1.5 symbolic retention and pruning
  outcomes.
...
- v1.3 Sprint 1 retained RegistryHub-local stream-offer status transition
  history for explicit symbolic lifecycle changes.
- v1.3 Sprint 2 read-only stream-offer expiration and lifecycle planning
  helpers over retained held offers.
- v1.3 Sprint 3 explicit caller-driven lifecycle plan application helpers and
  copied JSON-safe apply result summaries.
- v1.3 Sprint 4 scenario DSL actions and assertions for lifecycle planning,
  explicit apply results, and retained transition history.
- v1.3 scenarios `058` through `060` for expiration planning, apply with
  retained transition recording, and apply without transition recording.
- v1.3 Sprint 5 detailed snapshot visibility for lifecycle plan action
  results, lifecycle apply action results, and retained transition history,
