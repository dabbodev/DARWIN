...
DSL coverage through `066` and release-candidate hardening for
documentation/readiness checks. Release prep set the package and CLI version
to `darwin-sim 1.5.0`.

This release line remains symbolic simulator metadata flow only. It must not
become real networking, a network service, production DDoS protection, a
firewall, a privacy or anonymity system, DNS, an external service, real
cryptography, production E2EE, a delivery enforcement layer, or a background
cleanup system.
...
Implemented v1.5 work includes:
...
v1.5 work preserves these compatibility expectations:
...
claiming package publication, release assets, or production behavior.
...
The v1.5 release adds scenarios `064` through `066`:
...
The released scenario set is contiguous from `001` through `066`. v1.5
reports package and CLI version `darwin-sim 1.5.0`.

## Current Limitations

- v1.5.0 is released on `main` as `darwin-sim 1.5.0`.
- Sprint 1 through Sprint 6 work has been implemented and released.
- Focused helper and scenario tests have been added.
- v1.5 scenarios `064` through `066` have been released.
- No package publication or release asset upload was performed for v1.5.
- The package and CLI version report `darwin-sim 1.5.0`.

## Non-Goals

v1.5 release does not add:
...
v1.5.0 release. Release prep set the package and CLI version to
`darwin-sim 1.5.0` and added the changelog release entry. No package
publication or release asset upload was performed for v1.5.

Final validation passed `python -m ruff check .`, `python -m pytest` with 862
tests, `python scripts/run_all_scenarios.py` for scenarios `001` through
`066`, and `python -m darwin.cli.main --version` reporting
`darwin-sim 1.5.0`.
