...
from darwin.registry.retained_audit import (
    apply_retained_audit_compaction_batch as apply_audit_compaction_batch_op,
)
from darwin.registry.retained_audit import (
...
def _step_apply_retained_audit_compaction_batch(
    world: World,
    fields: dict[str, Any],
) -> None:
    hub = world.registry_hubs[str(fields["registry_hub"])]
    ...
    for history_type in SUPPORTED_RETAINED_AUDIT_HISTORY_TYPES:
        ...
        if decision is None:
            raise AssertionError("required batch decision reference was not resolved")
        decisions.append(decision)

    result = apply_audit_compaction_batch_op(
        hub,
        decisions,
    ...
    world.log(
        (
            "applied retained audit compaction batch "
            f"{result.batch_id} at {hub.hub_id}: "
            f"{result.compacted_count} compacted"
        ),
        event_type="retained_audit_compaction_batch_applied",
        target=hub.hub_id,
        hub_id=hub.hub_id,
        status="applied",
        data=result.to_summary(),
...
def _retained_audit_records(
    ...
    *,
    ...
    for history_type in ordered_history_types:
        ...
        elif history_type == "encryption_policy_decision":
